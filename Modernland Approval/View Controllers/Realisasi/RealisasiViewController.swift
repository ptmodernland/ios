...
//  Modernland Approval
//
//  Created by Aritio on 24/05/21.
//  Copyright © 2021 Modernland. All rights reserved.
//

import UIKit

class RealisasiViewController: BaseViewController {
    ...
    let buttonTitleList = ["Waiting Approval for REALISASI",
                           "History Approval for REALISASI"]
    var notif = ["0","0"]
...
extension RealisasiViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if buttonTitleList[indexPath.row].contains("Waiting") {
            let vc = StoryboardScene.Realisasi.listRealisasiViewController.instantiate()
                self.navigationController?.pushViewController(vc, animated: true)
        }
        if buttonTitleList[indexPath.row].contains("History") {
            let vc = StoryboardScene.Realisasi.listHistoryRealisasiViewController.instantiate()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
