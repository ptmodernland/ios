...
class KasbonViewController: BaseViewController {
    ...
    let buttonTitleList = ["Waiting Approval for KASBON",
                           "History Approval for KASBON"]
    var notif = ["0","0"]
...
extension KasbonViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if buttonTitleList[indexPath.row].contains("Waiting") {
            let vc = StoryboardScene.Kasbon.listKasbonViewController.instantiate()
                self.navigationController?.pushViewController(vc, animated: true)
        }
        if buttonTitleList[indexPath.row].contains("History") {
            let vc = StoryboardScene.Kasbon.listHistoryKasbonViewController.instantiate()
            self.navigationController?.pushViewController(vc, animated: true)
        }
    }
